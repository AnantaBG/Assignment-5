  ...
  for (const data of datas) {
    ...
    card.innerHTML = `
      ...
      <div class="px-3 items-left text-left">
        <div>
          <h2 class="text-xl pt-2 pb-6 font-bold">${data.pet_name}</h2>
        </div>
...
<defs>
 ...
 <p class = flex>
	...
	<g clip-path="url(#clip2081_59)">
      ...
      <div class="border-t flex justify-between pt-3 pb-2 like-button-container">
        <button id="like-btn-${data.id}" class="btn btn-sm border-teal-300">
          <img id="like-btn" class="w-[40px] h-[30px]" src="like.png">
        </button>
        <button onclick="my_modal_1" id="adopt-${data.id}" class="btn btn-sm border-teal-300  text-teal-500 ">
          Adopt
        </button>
        <button onclick="my_modal_2" id="details-${data.id}" class="banner btn btn-sm border-teal-300  text-teal-500  ">Details</button>
      </div>
    `;
...
//     innerCard.classList = "card bg-teal-600 ml-2 mb-2 border";
// 	 innerCard.innerHTML = `
...
// 	myModal2.append(innerCard)
    ...
    allData.append(card);
    const likeButton = card.querySelector(`#like-btn-${data.id}`);
	const adoptButton = card.querySelector(`#adopt-${data.id}`);
	const detailsButton = card.querySelector(`#details-${data.id}`);

    likeButton.addEventListener("click", () => handleLikeClick(data.image));
	...
	// adoptButton.addEventListener("click", () => handleAdoptClick(data.image));
	detailsButton.addEventListener("click", () => handleDetailsClick(data.image));
  }
};

const likeContainer = document.getElementById("selection-Container");

const handleLikeClick = (imageUrl) => {
  const likedImage = document.createElement("img");
  likedImage.src = imageUrl;
  likedImage.classList = "liked-image h-fit  rounded-xl pb-2 pr-2";
  likeContainer.appendChild(likedImage);
};
// const handleAdoptClick = (imageUrl) => {
// 	const likedImage = document.createElement("img");
// 	likedImage.src = imageUrl;
// 	likedImage.classList = "liked-image h-fit  rounded-xl pb-2 pr-2";
// 	likeContainer.appendChild(likedImage);
//   };
//   const handleDetailsClick = (imageUrl) => {
// 	const likedImage = document.createElement("img");
// 	likedImage.src = imageUrl;
// 	likedImage.classList = "liked-image h-fit  rounded-xl pb-2 pr-2";
// 	likeContainer.appendChild(likedImage);
