const selected = document.getElementById("selection-Container");
	

// console.log("Universal Es6");
const loadData = () => {
    // fetchdata
...
const displayData = (datas) => {
    const allData = document.getElementById("cards-Container");
    allData.innerHTML = ""
// console.log(datas);
for(const data of datas){
    ...
    // card.innerText = data.date_of_birth;
    card.innerHTML = `
  ...
  <div class="px-3  items-left text-left">
    ...
    <div>
    <h2 class="text-xl pt-2 pb-6 font-bold">${data.
        pet_name}</h2>
  </div>
    ...
    <div class="border-t flex justify-between pt-3 pb-2">
      ...
      <button id="like-btn${i}" class="btn btn-sm border-teal-300">

	  <img id="like-btn${i}" class="w-[40px] h-[30px]" src="like.png">
</button>
      <button id="Helio${i}" class="btn btn-sm border-teal-300  text-teal-500 ">
      Adopt</button>
      <button id="kalo${i}" class="banner btn btn-sm border-teal-300  text-teal-500  ">Details</button>
    </div>
  </div>
    `
	...
	// 	}
	// 	 else if (event.target.id === "Helio") {
	// 		console.log("Adopt button clicked!");
	// 		event.stopPropagation();
	// 	 }
	// 	 else if (event.target.id === "kalo") {
	// 		console.log("Details button clicked!");
	// 		event.stopPropagation();
	// 	 }
	// }); 
    // add cards to all data
	card.querySelector("#like-btn" + i).addEventListener("click", function() {
		console.log("Like button clicked for card", i);
	});

	card.querySelector("#adopt-btn" + i).addEventListener("click", function() {
		console.log("Adopt button clicked for card", i);
	});

	card.querySelector("#details-btn" + i).addEventListener("click", function() {
		console.log("Details button clicked for card", i);
	});
...
loadData();
// loadCategoryData();
// const allData = document.getElementById("cards-Container"); // Assuming the ID of the container
// const selectionContainer = document.getElementById("selection-Container"); // Assuming the ID of the next container

// allData.addEventListener("click", (event) => {
//   if (event.target.tagName.toLowerCase() === "button" && event.target.classList.contains("first-button")) {
//     // Ensure it's the first button and not another button
//     const card = event.target.parentElement; // Get the card element
//     const cardImage = card.querySelector("img"); // Find the image within the card

//     if (cardImage) {
//       const clonedImage = cardImage.cloneNode(true); // Clone the image
//       nextContainer.appendChild(clonedImage); // Add the cloned image to the next container
//     } else {
//       console.error("Image not found in the card!"); // Handle potential image absence
//     }
//   }
// });